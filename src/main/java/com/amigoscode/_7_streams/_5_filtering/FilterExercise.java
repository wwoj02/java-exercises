...
public class FilterExercise {
    ...
    public static void main(String[] args) {
        ...
        people.stream()
                ...
                .forEach(person -> System.out.printf(
                        "Name: %s, age: %d%n", person.name(), person.age()));
        ...
        orders.stream()
                ...
                .forEach(o -> System.out.printf("Order: %s, amount: %.2f%n",
                        o.product(), o.amount()));
        ...
        people.stream()
                .filter(p -> p.age() > 18 && p.city().equals("London"))
                .forEach(System.out::println);
        ...
        orders.stream().filter(isExpensive).forEach(order -> System.out.println(order.product()));
        ...
        //           Print each matching person
        people.stream()
                .filter(person -> person.city().equals("Paris") || person.age() < 18)
                .forEach(System.out::println);
        ...
        long n = orders.stream()
                .filter(p -> p.status.equals("COMPLETED"))
                .count();
