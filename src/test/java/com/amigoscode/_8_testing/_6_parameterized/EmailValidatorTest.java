...
class EmailValidatorTest {
    ...
    @ValueSource(strings = {"plaintext", "@missing-local.com",
            "missing-domain@", "missing@dot", "user@@double.com"})
    void TODO2(String invalidEmail) {
        var actual = emailValidator.isValid(invalidEmail);
        assertFalse(actual);
    }
    ...
    void TODO4(String email, boolean expected) {
        var actual = emailValidator.isValid(email);
    ...
    static Stream<Arguments> emailProvider() {
        return Stream.of(
                Arguments.of("valid@test.com", true),
                Arguments.of("invalid", false),
                Arguments.of("moc.@lion", false),
                Arguments.of("elephant11@", false)
        );
    }

    @ParameterizedTest
    @MethodSource("emailProvider")
    void TODO5(String email, boolean expected) {
        var actual = emailValidator.isValid(email);
    ...
    void TODO6(EmailDomain emailDomain) {
        String email = "test@" + emailDomain.domain;
